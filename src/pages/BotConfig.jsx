...
import { 
  Save, ArrowLeft, MessageSquare, Key, Headphones, // 🔥 Adicionado Headphones
...
export function BotConfig() {
  ...
  const [config, setConfig] = useState({
    ...
    id_canal_vip: '', 
    admin_principal_id: '',
    suporte_username: '', // 🔥 Campo presente
    status: 'desconectado'
  });
  ...
  const carregarDados = async () => {
    try {
      ...
      const currentBot = bots.find(b => b.id === parseInt(id)); // Ajuste de tipo
      ...
      if (currentBot) {
        setConfig({
          ...
          suporte_username: currentBot.suporte_username || '', // 🔥 Carrega suporte
  ...
  return (
    <div className="bot-config-container">
      <div className="config-header">
        ...
        <h1>Configuração do Bot: <span style={{color:'#c333ff'}}>{config.nome}</span></h1>
      </div>

      {/* TABS NAVIGATION */}
      <div className="config-tabs">
          <button className={`tab-btn ${activeTab === 'geral' ? 'active' : ''}`} onClick={() => setActiveTab('geral')}>
              <MessageSquare size={18}/> Geral
          </button>
          <button className={`tab-btn ${activeTab === 'miniapp' ? 'active' : ''}`} onClick={() => setActiveTab('miniapp')}>
                ...
                <Card>
                  <CardContent>
                    ...
                    <div className="form-group">
                      ...
                      {/* 🔥 CORREÇÃO 1: TOKEN LIBERADO PARA EDIÇÃO */}
                      <input 
                        className="input-field" 
                        value={config.token} 
                    ...
                    {/* 🔥 CORREÇÃO 2: CAMPO DE SUPORTE RESTAURADO */}
                    <div className="form-group">
                    ...
                    <div className="form-group">
                      <label><Radio size={16} style={{verticalAlign:'middle', marginRight:'5px'}}/> ID do Canal VIP</label>
                      <input 
                        className="input-field" 
                    ...
                    <div className="action-row" style={{marginTop: 30}}>
